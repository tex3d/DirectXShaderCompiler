...
enum class OpCode : unsigned {
  ...
  RawBufferVectorStore = 304, // writes to a RWByteAddressBuffer or RWStructuredBuffer
  ...
  GetRemainingRecursionLevels = 253, // returns how many levels of recursion remain
  IncrementOutputCount = 240, // Select the next logical output count for an EmptyNodeOutput for the whole group or per thread.
  NodeOutputIsValid = 252, // returns true if the specified output node is present in the work graph
  ...
  NumOpCodes_Dxil_1_9 = 306,

  NumOpCodes = 306 // exclusive last value of enumeration
};
// OPCODE-ENUM:END
