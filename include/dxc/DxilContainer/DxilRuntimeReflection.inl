...
template<typename _RecordType> //, typename _std_ostream>
bool ValidateRecord(const RDATContext &ctx, const _RecordType *pRecord) { //, _std_ostream &error_stream) {
  return false;
}
...
class RecursiveRecordValidator {
  ...
  //_std_ostream &m_ErrorStream;
public:
  RecursiveRecordValidator(const hlsl::RDAT::RDATContext &ctx, uint32_t recordStride) //, _std_ostream &error_stream)
      : m_Context(ctx), m_RecordStride(recordStride) {} //, m_ErrorStream(error_stream) {}
  template<typename _RecordType>
  bool Validate(const _RecordType *pRecord) const {
    if (sizeof(_RecordType) <= m_RecordStride) {
      if (!ValidateRecord(m_Context, pRecord))
        return false;
      return ValidateDerived(pRecord);
    }
    return true;
  }
...
template<typename _RecordType> //, typename _std_ostream>
static bool ValidateRecordTable(RDATContext &ctx, RecordTableIndex tableIndex) { //, _std_ostream &error_stream) {
...
//template<typename _std_ostream>
bool DxilRuntimeData::Validate() { //_std_ostream &error_stream) {
...
#undef RDAT_STRUCT_TABLE
#define RDAT_STRUCT_TABLE(type, table) ValidateRecordTable<type>(m_Context, RecordTableIndex::table); //, error_stream);
