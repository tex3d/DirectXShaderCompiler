...
class db_oload_gen:
    ...
    def print_opfunc_props(self):
        ...
        oload_to_mask = lambda oload: sum([1 << dxil_all_user_oload_chars.find(c) for c in oload])
        maybe_oloads_fn = lambda oloads: oloads if oloads else []
        ext_oload_fn = lambda i: "{" + ",".join([
            "0x%02x" % oload_to_mask(o)
            for o in maybe_oloads_fn(i.extended_oload_types)
            ]) + "}"
        vec_oload_fn = lambda i: "{" + ",".join([
            "0x%02x" % oload_to_mask(o)
    ...
    def print_opfunc_oload_type(self):
        ...
        for index_tuple, opcodes in extended_dict.items():
            ...
            if index_tuple[-1] > 0:
                line += (f"  if (FT->getNumParams() < {index_tuple[-1]})\n"
                    + "    return nullptr;\n")
            line += ("  return llvm::StructType::get(Ctx, {"
