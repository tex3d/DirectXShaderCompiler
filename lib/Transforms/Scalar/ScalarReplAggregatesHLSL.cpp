...
bool replaceScalarArrayWithVectorArray(Value *ScalarArray, Value *VectorArray,
  ...
  for (auto it = ScalarArray->user_begin(); it != ScalarArray->user_end();) {
    ...
    if (BitCastInst *BCI = dyn_cast<BitCastInst>(U)) {
      // FIXME: This will replace the memcpy dest! We should only replace things
      // that are properly dominated by the memcpy.  Since we may fail to
      // replace some uses, messing up the memcpy will mess up the fallback path
      // of splitting the memcpy because dest will never be copied into, and
      // users of dest will load the wrong values.
      // At least, avoid replacing the dest of the memcpy.
