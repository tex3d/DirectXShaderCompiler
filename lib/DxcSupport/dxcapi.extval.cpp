...
public:
  ...
  void ProcessArgs(LPCWSTR **Arguments, UINT32 *ArgCount,
    ...
    hlsl::options::MainArgs MainArgs(static_cast<int>(*ArgCount), *Arguments, 0);
    ...
    if (ProduceDxModule) {
      ...
      // Add argument to disable validation, so we can call it ourselves later.
      if (NeedsValidation)
        AddArgument(L"-Vd");
    }
...
private:
  ...
  // for the requested interface, which wraps the result of QueryInterface on
  // the compiler object. Compiler pointer is held as IUnknown and must be
  // upcast to the appropriate interface on use.
  IID m_CompilerIID;
...
public:
  ...
  // retrieved by calling QueryInterface on the compiler. The new wrapper keeps
  // the IID and returned compiler interface pointer for use in associated
  // interface methods. Those methods can then use castCompilerUnsafe to upcast
  // the pointer to the appropriate interface for the method.
  HRESULT STDMETHODCALLTYPE QueryInterface(REFIID iid,
    ...
    *ppvObject = nullptr;
    ...
    try {
      // This can throw; do not leak C++ exceptions through COM method calls.
